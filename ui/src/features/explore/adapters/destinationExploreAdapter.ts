...
function normalizeForSearch(destination: Destination): string {
  return [
    destination.name,
    destination.country,
    destination.region,
...
export function getDestinations(
  ...
  return CATALOG.filter((destination) => {
    const matchesQuery = normalizedQuery
      ? normalizeForSearch(destination).includes(normalizedQuery)
      : true;
    const matchesMood = mood ? destination.moods.includes(mood) : true;
...
export function getRecommendedForPlannedDestinations(
  plannedDestinations: string[],
): Destination[] {
  const normalizedPlanned = plannedDestinations.map((d) => d.toLowerCase().trim());

  const unplanned = CATALOG.filter(
    (d) => !normalizedPlanned.some((p) => p.includes(d.name.toLowerCase())),
  );

  return unplanned.slice(0, 4);
}
