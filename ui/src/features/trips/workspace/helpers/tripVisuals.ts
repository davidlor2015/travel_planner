...
function destinationParts(destination: string) {
  const [city, country] = destination.split(',').map((value) => value.trim()).filter(Boolean);
  return {
...
export function getTripImageUrl(trip: Pick<Trip, 'id' | 'destination'>): string {
...
export function getTripTagline(trip: Pick<Trip, 'destination' | 'start_date' | 'end_date'>): string {
  const { city, country } = destinationParts(trip.destination);
  ...
  return `${nights} night${nights === 1 ? '' : 's'} in ${city}, with room to wander.`;
}

export function getDestinationLabel(destination: string) {
  return destinationParts(destination).city;
}
