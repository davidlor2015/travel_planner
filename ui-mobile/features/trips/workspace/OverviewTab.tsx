...
export function OverviewTab({ tripId, trip, summary }: Props) {
  ...
  return (
    <ScrollView contentContainerClassName="gap-4 px-4 pb-8 pt-4">
      ...
      <SectionCard
        ...
        title="Group"
        description="The mobile shell stays focused on who is in the trip and whether the trip owner is you."
      >
        <View className="gap-2">
          <View className="flex-row flex-wrap gap-2">
            <StatusPill label={trip.statusLabel} variant={trip.status === "active" ? "success" : trip.status === "upcoming" ? "warning" : "default"} />
            <StatusPill label={trip.isOwner ? "You own this trip" : "Shared trip"} variant="info" />
          </View>
          {trip.members.map((m) => (
            <View key={m.userId} className="rounded-2xl border border-border bg-surface-muted px-3 py-3">
              <Text className="text-sm font-medium text-text">{m.email}</Text>
